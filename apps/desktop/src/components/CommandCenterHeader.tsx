          ...
          <CardContent className="p-3 py-1.5">
            ...
            >
              <div className="flex items-center gap-2 shrink-0">
                <Radio className="h-4 w-4 text-red-500 animate-pulse-glow" style={{ filter: 'drop-shadow(0 0 4px rgba(239, 68, 68, 0.7))' }} />
                <span className="text-xs font-semibold text-red-400" style={{ textShadow: '0 0 8px rgba(239, 68, 68, 0.5)' }}>Live Feed</span>
              </div>
                  ...
                  {[...news, ...news].map((item, i) => (
                    ...
                    >
                      <span className={`inline-block w-1.5 h-1.5 rounded-full shrink-0 ${
                        item.category?.toLowerCase().includes('vuln') || item.category?.toLowerCase().includes('cve') ? 'bg-red-400' :
                        item.category?.toLowerCase().includes('breach') || item.category?.toLowerCase().includes('ransom') ? 'bg-orange-400' :
                        item.category?.toLowerCase().includes('apt') || item.category?.toLowerCase().includes('threat') ? 'bg-yellow-400' :
                        'bg-primary/60'
                      }`} />
                      <span className="text-gray-200 group-hover:text-white transition-colors">{item.title}</span>
                      <span className="text-[10px] text-gray-400">{item.source} · {item.time}</span>
                    </a>
                  ))}
                </div>
              </div>
