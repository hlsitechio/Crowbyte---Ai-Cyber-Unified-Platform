...
export default function NetworkScanner() {
 ...
 disabled={isScanning}
 className="font-mono text-xs h-8 bg-transparent max-w-sm"
 onKeyDown={e => { if (e.key === 'Enter' && !isScanning) runScan(); }}
 />
 <Select value={selectedProfile} onValueChange={setSelectedProfile}>
 <SelectTrigger className="w-28 h-8 text-[11px] bg-transparent">
 <SelectValue />
 </SelectTrigger>
 <SelectContent>
 {SCAN_PROFILES.map(p => (
 ...
 className="font-mono text-xs h-8 bg-transparent max-w-xs"
 />
 )}
 <Button
 onClick={runScan}
 ...
 >
 <Controls className="!bg-zinc-900 !border-zinc-700 !rounded-lg [&>button]:!bg-zinc-900 [&>button]:!border-zinc-700 [&>button]:!text-zinc-300 [&>button:hover]:!bg-zinc-700" />
 <MiniMap
 ...
 <div className="bg-zinc-900/90 backdrop-blur-sm border rounded-lg px-3 py-2">
 ...
 >
 <div className={`w-2 h-2 rounded-full ${d.status === 'up' ? 'bg-emerald-500' : d.status === 'down' ? 'bg-red-500' : 'bg-zinc-500'}`} />
 ...
 {d.os && <span className="text-[10px] text-blue-500/70">{d.os}</span>}
 </div>
 </div>
