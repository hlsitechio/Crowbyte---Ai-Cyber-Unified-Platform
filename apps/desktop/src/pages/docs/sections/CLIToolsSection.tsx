...
export function CLIToolsSection() {
  return (
    <div className="space-y-6">
      <DocHeader icon={TerminalSquare} title="CLI Tools" description="cve-db and kb command-line tools shared across all Claude Code instances" status="ready" />
      ...
      <Card><CardHeader><CardTitle>cve-db</CardTitle><CardDescription>/usr/local/bin/cve-db — CVE lookup, search, and management</CardDescription></CardHeader>
        <CardContent>
          <CodeBlock>
            ...
            <div><span className="text-green-400">cve-db nvd</span> CVE-2024-3400      <span className="text-zinc-500"># NVD API v2.0 only</span></div>
            <div><span className="text-green-400">cve-db shodan</span> CVE-2024-3400   <span className="text-zinc-500"># Shodan CVEDB only</span></div>
            <div>&nbsp;</div>
            <div className="text-zinc-500"># Database search and filtering</div>
            <div><span className="text-green-400">cve-db search</span> "RCE"            <span className="text-zinc-500"># Full-text search</span></div>
            <div><span className="text-green-400">cve-db list</span> --severity CRITICAL <span className="text-zinc-500"># Filter by severity</span></div>
            <div><span className="text-green-400">cve-db list</span> -n 20              <span className="text-zinc-500"># Last 20 entries</span></div>
            <div><span className="text-green-400">cve-db stats</span>                   <span className="text-zinc-500"># Severity breakdown</span></div>
            <div>&nbsp;</div>
            ...
            <div>  <span className="text-green-400">cve-db lookup</span> "$cve"</div>
            <div>done</div>
          </CodeBlock>
        </CardContent></Card>
      ...
      <Card><CardHeader><CardTitle>kb</CardTitle><CardDescription>/usr/local/bin/kb — Knowledge base save, search, and pipe</CardDescription></CardHeader>
        <CardContent>
          <CodeBlock>
            ...
            <div><span className="text-green-400">kb search</span> "RCE"          <span className="text-zinc-500"># Full-text search</span></div>
            <div><span className="text-green-400">kb recent</span> -n 10          <span className="text-zinc-500"># Last 10 entries</span></div>
            <div><span className="text-green-400">kb list</span> --category tools <span className="text-zinc-500"># Filter by category</span></div>
          </CodeBlock>
        </CardContent></Card>
